...
def run_one_sim(lower_corner, cube_size, show_gui=False):
    ...
    mpm.add_cube(lower_corner=lower_corner, cube_size=cube_size,
...
for sim_idx in range(args.n_sims):
    ...
    lower_corner, cube_size = sample_cube_params()
    print(f"Running simulation {sim_idx}: lower_corner={lower_corner}, cube_size={cube_size}")

    pos_arr, vel_arr, mat_arr = run_one_sim(lower_corner, cube_size, show_gui=args.show_gui)
        ...
        with open(pt_path, 'ab') as f_pt, open(pos_path, 'ab') as f_pos:
            ...
            for mat in unique_mats:
                ...
                offset[key] = {
                    ...
                    "params": {
                        "lower_corner": [float(lower_corner[0]), float(lower_corner[1])],
                        "cube_size": [float(cube_size[0]), float(cube_size[1])]
                    }
                }
